...
public interface StatRepository extends JpaRepository<EndpointHit, Integer> {
    @Query("select new ru.practicum.dto.ViewStatsDto(h.app, h.uri, count(h.ip)) from EndpointHit h " +
    ...
    @Query("select new ru.practicum.dto.ViewStatsDto(h.app, h.uri, count(distinct h.ip)) from EndpointHit h " +
            "where h.timestamp between :start and :end " +
            "group by  h.app, h.uri " +
            "order by h.app desc ")
